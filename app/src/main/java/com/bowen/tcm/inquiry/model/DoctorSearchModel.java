...
public class DoctorSearchModel extends BaseModel {

    public DoctorSearchModel(Context mContext) {
        super(mContext);
    }
    ...
    public void showHotSearchList(final HttpTaskCallBack<List<String>> mListener) {
        ...
        networkRequest.requestSRV(HttpContants.CMD_SHOW_HOT_SEARCH_DOCTOR_LIST, "", new HttpTaskCallBack<Object>() {
            @Override
    ...
    public void showUserSerachLog(final HttpTaskCallBack<List<String>> mListener) {
        ...
        networkRequest.setParam("token", UserInfo.getInstance().getToken());
        networkRequest.setParam("userId", UserInfo.getInstance().getUserId());
        networkRequest.requestSRV(HttpContants.CMD_SHOW_USER_SERACH_DOCTOR_LOG, "", new HttpTaskCallBack<Object>() {
            @Override
            public void onSuccess(HttpResult<Object> httpResult) {
                HttpResult<List<String>> result = new HttpResult<>();
                result.copy(httpResult);
    ...
    public void deleteUserSearchLog(final HttpTaskCallBack<List<String>> mListener) {
        ...
        networkRequest.setParam("token", UserInfo.getInstance().getToken());
        networkRequest.setParam("userId", UserInfo.getInstance().getUserId());
        networkRequest.requestSRV(HttpContants.CMD_DELETE_USER_SEARCH_DOCTOR_LOG, "提交请求中", new HttpTaskCallBack<Object>() {
            @Override
            public void onSuccess(HttpResult<Object> httpResult) {
                HttpResult<List<String>> result = new HttpResult<>();
                if (mListener != null) {
